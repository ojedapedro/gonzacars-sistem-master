...
import { AccountReceivable, ReceivablePayment, PaymentMethod } from '../types';
import { Wallet, Search, Filter, Plus, ChevronRight, X, Calendar, DollarSign, FileText, CheckCircle } from 'lucide-react';
...
const AccountsReceivableModule: React.FC = () => {
  ...
  const totalReceivables = accounts.reduce((acc, curr) => acc + curr.totalAmount, 0);
  const totalPaid = accounts.reduce((acc, curr) => acc + curr.paidAmount, 0);
  const totalPending = totalReceivables - totalPaid;

  const handleOpenPayment = (acc: AccountReceivable) => {
  ...
  const handleRegisterPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Abono registrado', 'El pago se ha registrado correctamente.');
    ...
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-chrome-400 mt-1">Gestión de deudas y abonos de clientes.</p>
        </div>
        
        <div className="flex gap-4">
          <div className="bg-metal-800 p-4 rounded-xl border border-metal-700/50 min-w-[150px]">
            <p className="text-chrome-500 text-xs font-semibold uppercase tracking-wider mb-1">Por Cobrar</p>
            <p className="text-2xl font-bold text-blue-400">{formatCurrency(totalPending)}</p>
          </div>
          <div className="bg-metal-800 p-4 rounded-xl border border-metal-700/50 min-w-[150px]">
            <p className="text-chrome-500 text-xs font-semibold uppercase tracking-wider mb-1">Cobrado</p>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-metal-900 border border-metal-800 rounded-xl pl-10 pr-4 py-3 text-chrome-100 placeholder-chrome-500 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
        <div className="relative min-w-[200px]">
          ...
          <select
            ...
            className="w-full bg-metal-900 border border-metal-800 rounded-xl pl-10 pr-4 py-3 text-chrome-100 focus:outline-none focus:border-blue-500 appearance-none"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAccounts.map(acc => (
          <div key={acc.id} className="bg-metal-800/50 border border-metal-700/50 rounded-2xl p-5 hover:border-metal-600 transition-colors">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-bold text-chrome-100 text-lg">{acc.customerName}</h3>
            ...
            <div className="space-y-3 mb-5">
              <div className="flex justify-between items-center text-sm">
                <span className="text-chrome-400">Total Deuda:</span>
                <span className="font-semibold text-chrome-200">{formatCurrency(acc.totalAmount)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-chrome-400">Abonado:</span>
              ...
              <div className="flex justify-between items-center text-sm pt-1 border-t border-metal-700/50 mt-2">
                ...
                <span className="font-bold text-amber-400">{formatCurrency(acc.totalAmount - acc.paidAmount)}</span>
              </div>
            </div>
            ...
            {acc.status !== 'Pagado' && (
              <button
                ...
                className="w-full py-2.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 font-semibold rounded-xl border border-blue-500/30 transition-colors flex items-center justify-center gap-2"
              >
            ...
            {acc.status === 'Pagado' && (
              <div className="w-full py-2.5 bg-emerald-500/10 text-emerald-400 font-semibold rounded-xl border border-emerald-500/20 flex items-center justify-center gap-2">
                <CheckCircle size={18} /> Deuda Saldada
              </div>
            )}
          </div>
        ))}
        {filteredAccounts.length === 0 && (
          <div className="col-span-full py-12 text-center bg-metal-800/30 rounded-2xl border border-metal-700/50 border-dashed">
            <Wallet size={48} className="mx-auto text-chrome-600 mb-4" />
            <p className="text-chrome-400 text-lg">No hay cuentas por cobrar que coincidan con la búsqueda.</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-metal-900 border border-metal-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleRegisterPayment} className="p-6 space-y-4">
              <div className="bg-metal-800/50 p-4 rounded-xl border border-metal-700/50 mb-4 text-center">
                <p className="text-chrome-400 text-sm">Deuda Restante de {selectedAccount.customerName}</p>
              ...
              <div>
                <label className="block text-sm font-medium text-chrome-400 mb-1">Monto a Abonar ($)</label>
                <input
                  type="number"
                  step="0.01"
                  required
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-metal-800">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-colors shadow-lg shadow-emerald-900/20"
                >
                  Confirmar Pago
                </button>
              </div>
